...
exports.queryRunner = function (dbpath, channelid, callback) {
    /**
     ...
     * Then opens the file `data.js` and prints the object as a JSON string.
    ...
    let db = new sqlite3.Database(dbpath, function (err) {
        if(err) {
            ...
            fs.writeFile('data.js', 'data = null', function (err) {
                if(err) {
    ...
    db.all('select * from messages where channel_id = ' + channelid, function (err, obj) {
        if(err) {
            ...
            fs.writeFile('data.js', jstr, function (err) {
                if(err) {
                    console.error("Welp! Can't write to the data.js file.");
                    console.error("This is going to lead to some nasty results on the webpage...");
                }
            });
        }
